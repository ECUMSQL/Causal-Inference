tsset id session //设置面板数据
foreach x of varlist lnw age asq bmi hispanic black other asian schooling cohab married divorced separated age_cl unsafe llength {
	reg asq_cl appearance_cl provider_second asian_cl black_cl hispanic_cl othrace_cl hot massage_cl
	}  // 不停重复回归  合并最小二乘POLS
drop if x==.
bysort id: gen s=_N
keep if s==4
